...
def selectfileinit():
    ...
    if infiles:
        for file_path in infiles:
            ...
            wave_original = np.array(observation_data['wave'])
...
def update_initvals():
    ...
    wave_cnts = wave_original - (wave_original / cc * float(star_rv_entry.get()))
...
param2_frame.grid(row=16, column=0, rowspan=6, columnspan=5, sticky="nsew")
...
spandropd = ttk.Combobox(param2_frame, textvariable=spandropdowntext, values=spanoptionsvar, width=6)
...
#spanbutton = tk.Button(param2_frame, text="Change Mol", command=lambda: on_span_select((spanoptionsvar[spandropd.current()]).lower()))
...
# Function to open file from the GUI using Open File for "Spectrum data file"
def selectfile():
    global file_path
    global file_name
    global wave_cnts
